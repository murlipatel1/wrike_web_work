...
exports.getWebworkProjectId = async (wrikeProjectId, projectName, webworkUserId) => {
    try {
        ...
        if (existingMapping) {
            ...
            const webworkProjectAssignees = await getWebworkProjectAssignees(webworkProjectId);
        ...
        const webworkProjectId = await createWebworkProject(wrikeProjectId, projectName, webworkUserId);
...
async function addUserToWebworkProject(webworkProjectId, webworkUserId) {
    try {
        const response = await axios.post(
            ...
            {
                headers: {
                    'Authorization': `Basic ${config.webwork.apiUser}:${config.webwork.apiToken}`,
...
async function createWebworkProject(wrikeProjectId, projectName, webworkUserId) {
    try {
        ...
        const webworkProjectId = response.data.id;
        ...
        console.error('Error creating Webwork project:', error.response?.data || error.message);
...
exports.createWebworkTask = async (webworkProjectId, task, webworkUserid) => {
    try {
        ...
        const webworkTaskId = response.data.id;
        // assign user to task
        ...
        const user = await User.findOne({ wrikeId: webworkUserid });
        const email = user.email;

        // Save mapping to database
        await saveTaskMapping(wrikeTaskId, webworkTaskId,email, webworkProjectId, webworkUserid);
...
const saveTaskMapping = async (wrikeTaskId, webworkTaskId,email,webworkProjectId, webworkUserId) => {
    try {
...
exports.getTaskTime = async (webworkTask) => {
    ...
    const start_date = webworkTask.createdAt.toISOString().split('T')[0]; 
    start_date.setDate(start_date.getDate() - 1);
    const end_date = Date.now();
    end_date.setDate(end_date.getDate() + 1);

    //find email from db of user_wrike_webork by webworkTask.id
    const task = await Task.findOne({ webworkTaskId: webworkTask.id });
    ...
    try {
        ...
        const report = response.data.dateReport.find(item=>item.email===email);
        if (!report) {
            throw new Error('No report found for the given date range');
        }
        const task = report.tasks.find(item=>item.id===webworkTask.id);
        if (!task) {
            throw new Error('No task found in the report');
        }
        return task.minutes - (task.inactive_minutes || 0)
    }
    catch (error) {
...
exports.removeAssignee= async (webworkUserId, webworkProjectId) => {
    try {
        ...
        const tasksInProject = tasks.filter(task => task.webworkProjectId === webworkProjectId);
        if(tasksInProject.length === 0) {
            // make function to get all contracts of the user
            const contracts = await getContractsofProject(webworkProjectId,webworkUserId);
...
const getContractsofProject = async (webworkProjectId, webworkUserId) => {
    try {
        ...
        const contracts = response.data.filter(contract => contract.project_id === webworkProjectId && contract.user_id === webworkUserId);
        return contracts;
